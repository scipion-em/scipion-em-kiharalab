...
class ProtEmap2sec(EMProtocol):
    ...
    def createOutputStep(self):
        ...
        if self.executionType.get() == EMAP2SEC_TYPE_EMAP2SECPLUS: # TMP, Need to get output structure
            print("TEST - ", self.getOutputFile(self.getVolumeAbsolutePaths()[0]))
            return
        ...
        inputVolumes = self.inputVolumeEmap2sec.get()
    ...
    def getOutputPath(self):
        ...
        if self.executionType.get() == EMAP2SEC_TYPE_EMAP2SECPLUS:
            typePath = ('SIMU6' if self.mode.get() == EMAP2SECPLUS_TYPE_SIMUL6A else
                ('SIMU10' if self.mode.get() == EMAP2SECPLUS_TYPE_SIMUL10A else
                ('SIMU_MIX' if self.mode.get() == EMAP2SECPLUS_TYPE_SIMUL6_10A else 'REAL'))
            )
            foldPath = 'Fold{}_Model_Result'.format(self.getFoldModel())
            filePath = self.getVolumeNames()[0]
            outputPath = os.path.join(outputPath, typePath, foldPath, filePath)
    ...
    def getOutputFile(self, inputFile):
        ...
        """
        defaultName = (
            'visual' if self.executionType.get() == EMAP2SEC_TYPE_EMAP2SEC
            else ('{}Phase2_pred{}'.format(inputFile, ('C' if self.getConfident.get() else '')))
        ) + '.pdb'
        return os.path.join(self.getOutputPath(), self.getProtocolFilePrefix(inputFile)) + defaultName
