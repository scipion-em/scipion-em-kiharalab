...
# Plugin imports
from .. import Plugin
from ..constants import EMAP2SEC_TYPE_EMAP2SEC, EMAP2SEC_NORM_GLOBAL, EMAP2SECPLUS_MODE_DETECT_STRUCTS, EMAP2SEC_TYPE_EMAP2SECPLUS
from ..constants import EMAP2SECPLUS_TYPE_EXPERIMENTAL, EMAP2SECPLUS_MODE_DETECT_EVALUATE_STRUCTS, EMAP2SECPLUS_MODE_DETECT_DNA
from ..constants import EMAP2SECPLUS_FOLD3, EMAP2SECPLUS_MODE_DETECT_EXPERIMENTAL_FOLD4, EMAP2SECPLUS_MODE_DETECT_DNA_EXPERIMENTAL_FOLD4
from ..constants import EMAP2SECPLUS_MODE_DETECT_EVALUATE_EXPERIMENTAL_FOLD4, EMAP2SECPLUS_TYPE_SIMUL6A, EMAP2SECPLUS_TYPE_SIMUL10A, EMAP2SECPLUS_TYPE_SIMUL6_10A
...
class ProtEmap2sec(EMProtocol):
	...
	def runEmap2sec(self, args, outDir=None, clean=True):
		...
		envActivationCommand = f"{Plugin.getCondaActivationCmd()} {Plugin.getProtocolActivationCommand('emap2sec')}"
		...
		datasetCommand = f"{envActivationCommand} && python data_generate/dataset.py"
		...
		emap2secCommand = f"{envActivationCommand} && python emap2sec/Emap2sec.py"
		...
		if clean:
			for tmpFile in args[5]:
				self.runJob("rm -rf", tmpFile, cwd=Plugin._emap2secBinary)
	...
	def runEmap2secPlus(self, args, clean=True):
		...
		envActivationCommand = f"{Plugin.getCondaActivationCmd()} {Plugin.getProtocolActivationCommand('emap2sec', 'emap2secPlus')}"
		...
		# Emap2sec+ execution command
		runCommand = f"{envActivationCommand} && python3 main.py"
		self.runJob(runCommand, args[0], cwd=Plugin._emap2secplusBinary)

		# Output file/s relocation
		self.runJob("mv", f"{args[1][0]} {args[1][1]}", cwd=Plugin._emap2secplusBinary)
		if len(args[1]) == 4:
			self.runJob("mv", f"{args[1][2]} {args[1][3]}", cwd=Plugin._emap2secplusBinary)

		# Remove temporary files
		if clean:
			for tmpFile in args[2]:
				self.runJob("rm -rf", tmpFile, cwd=Plugin._emap2secplusBinary)
	# --------------------------- INFO functions -----------------------------------
	def _summary(self):
		"""
	...
	def getProtocolFilePrefix(self, filename):
		...
		"""
		return f'{self.getProtocolPrefix()}{os.path.splitext(self.getCleanVolumeName(filename))[0]}_'
	...
	def getConvertedVolumeRelativePath(self):
		...
		fileExtension = os.path.splitext(self.getVolumeAbsolutePath())[1]
		if fileExtension != '.mrc' and fileExtension != '.map':
			return self.scapePath(self._getTmpPath(self.getVolumeName(f"{os.path.splitext(self.getVolumeAbsolutePath())[0]}.mrc")))
		else:
	...
	def getOutputFile(self, inputFile):
		...
		filename = (
			(f"{self.getProtocolFilePrefix(inputFile)}visual.pdb") if self.executionType.get() == EMAP2SEC_TYPE_EMAP2SEC
	...
	def getFilesToRemove(self):
		...
		if self.executionType.get() == EMAP2SEC_TYPE_EMAP2SEC:
			...
			args.append(f'data/{filePrefix}trimmap') # Trimmap file
			args.append(f'data/{filePrefix}dataset') # Dataset file
			args.append(f'data/{protocolPrefix}input.txt') # Input location file
			for i in range(1, 3):
				args.append(f'results/{protocolPrefix}outputP{i}_{filePrefix}dataset') # Result prediction files
		else:
	...
	def getDatasetArgs(self):
		...
		"""
		outputPefix = f'data/{self.getProtocolFilePrefix(self.getVolumeAbsolutePath())}'
		proteinId = (' ' + self.proteinId.get()) if self.proteinId.get() else ''
		return f'{outputPefix}trimmap {outputPefix}dataset{proteinId}'
	...
	def getInputLocationFileArgs(self):
		...
		"""
		return f'\'data/{self.getProtocolFilePrefix(self.getVolumeAbsolutePath())}dataset\' > data/{self.getProtocolPrefix()}input.txt'
	
	def getEmap2secArgs(self):
		"""
		This method returns the arguments neccessary for the Emap2sec.py's execution.
		"""
		return f'data/{self.getProtocolPrefix()}input.txt --prefix results/{self.getProtocolPrefix()}'
	...
	def getCustomModel(self):
		...
		return (f' -M={self.scapePath(customModel)}' if 
	...
	def getEmap2secPlusArgs(self):
		...
		if executionMode != EMAP2SECPLUS_MODE_DETECT_DNA_EXPERIMENTAL_FOLD4:
			param += f' --class={self.classes.get()}'
		...
		if (self.mapType.get() == EMAP2SECPLUS_TYPE_EXPERIMENTAL and 
			...
			param += f' --fold={self.getFoldModel()}'
		...
		if executionMode == EMAP2SECPLUS_MODE_DETECT_EVALUATE_STRUCTS or executionMode == EMAP2SECPLUS_MODE_DETECT_EVALUATE_EXPERIMENTAL_FOLD4:
			param += f' -P={self.getStructAbsolutePath()}'
	...
	def getEmap2secPlusDefaultOutputPath(self, base=False):
		...
		foldPath = f'Fold{self.getFoldModel()}_Model_Result'
	...
	def getEmap2secPlusOutputFile(self, clean=True):
		...
		return (
			f'{volumeName}'
			f'{"Final" if self.getMode() == EMAP2SECPLUS_MODE_DETECT_DNA_EXPERIMENTAL_FOLD4 else "Phase2"}_pred'
			f'{"C" if self.getConfident.get() else ""}.pdb'
		)
	...
	def getEmap2secPlusMetricsFile(self, clean=True):
		...
		inputFile = self.getVolumeAbsolutePath()
		volumeName = os.path.splitext(self.getCleanVolumeName(inputFile) if clean else self.getVolumeName(inputFile))[0]
		return f'{volumeName}Phase2_report.txt'
